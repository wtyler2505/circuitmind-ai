...
import { augmentComponentData, identifyComponentFromImage, suggestProjectsFromInventory, findComponentSpecs, generateComponentThumbnail } from '../services/geminiService';
...
const getTypeIcon = (type: string) => {
  switch (type) {
    case 'microcontroller': return 'M';
    case 'sensor': return 'S';
    case 'actuator': return 'A';
    case 'power': return 'P';
    default: return 'O';
  }
};
...
const Inventory: React.FC<InventoryProps> = ({ 
  ...
  const handleAdd = () => {
    ...
    const pins = newItemPins.split(',').map(p => p.trim()).filter(p => p);
        ...
        <div className="p-4 md:p-5 border-b border-slate-800 bg-gradient-to-b from-slate-900 to-transparent flex flex-col gap-2">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold font-sans text-white mb-1 flex items-center gap-2">
                    <svg className="w-5 h-5 text-neon-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
                    ASSET MANAGER
                </h2>
                {/* Mobile Close Button */}
                <button
            ...
            <p className="text-[10px] text-slate-400 font-mono tracking-widest uppercase mb-2">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-grid-white/[0.02] relative">
            ...
            {activeTab === 'tools' && (
                <div className="p-4 space-y-6">
                    <div>
                        <h3 className="text-xs font-bold text-slate-400 mb-3 border-b border-slate-800 pb-1">DATA MANAGEMENT</h3>
                        <div className="grid grid-cols-2 gap-2">
                             <button onClick={handleExport} className="bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded border border-slate-600 flex flex-col items-center gap-1">
                                <svg className="w-5 h-5 text-neon-green" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
                             ...
                             <label className="bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded border border-slate-600 flex flex-col items-center gap-1 cursor-pointer">
                                ...
                                <svg className="w-5 h-5 text-neon-purple" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                        ...
                        {onReset && (
                          <div className="mt-2">
                            <button onClick={onReset} className="w-full bg-red-900/20 hover:bg-red-900/50 text-red-400 py-2 rounded border border-red-900/50 flex items-center justify-center gap-2 transition-colors">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    ...
                    <div>
                        <h3 className="text-xs font-bold text-slate-400 mb-3 border-b border-slate-800 pb-1">AI ANALYSIS</h3>
                        <button 
