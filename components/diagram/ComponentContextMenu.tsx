/**
 * ComponentContextMenu - Right-click context menu for diagram components.
 *
 * Extracted from MainLayout to reduce render complexity.
 * Provides Edit, Duplicate, Generate 3D, and Delete actions.
 */
...
interface ComponentContextMenuProps {
  ...
  /** Screen y position */
  y: number;
  /** ID of the right-clicked component */
  componentId: string;
  /** Current diagram (used to find the component) */
  diagram: WiringDiagram | null;
  /** Open component in the editor modal */
  onEdit: (component: ElectronicComponent) => void;
  /** Duplicate the component on the canvas */
  onDuplicate: () => void;
  /** Open component in editor modal for 3D generation */
  onGenerate3D: (component: ElectronicComponent) => void;
  /** Remove the component and its connections from the diagram */
  onDelete: () => void;
}

const ComponentContextMenuComponent: React.FC<ComponentContextMenuProps> = ({
  x,
  y,
  componentId,
  diagram,
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={() => component && onEdit(component)}
      >
        <svg
      ...
      <button
        className="w-full text-left px-4 py-2 text-xs font-medium text-slate-300 hover:bg-neon-cyan/10 hover:text-neon-cyan transition-colors flex items-center gap-2 group"
        onClick={() => component && onGenerate3D(component)}
      >
        <svg
          className="w-3.5 h-3.5 opacity-50 group-hover:opacity-100"
          fill="none"
...
export const ComponentContextMenu = memo(ComponentContextMenuComponent);
