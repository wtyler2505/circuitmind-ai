...
- **Security**: Execution is gated behind an explicit **RUN 3D CODE** confirmation in `ComponentEditorModal`. `ThreeViewer` rejects unsafe tokens and requires `return group;` before running.
